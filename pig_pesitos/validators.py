...
def validate_amount(text: str) -> tuple[bool, str | None, float | None]:
    ...
    if amount > MAX_AMOUNT:
        return False, "El monto es demasiado alto. El monto máximo permitido es 1,000,000:", None
...
def validate_limit_amount(text: str) -> tuple[bool, str | None, float | None]:
    ...
    if amount >= MAX_LIMIT_AMOUNT:
        return False, "El límite debe ser menor a $100,000.00. Intenta nuevamente:", None
    return True, None, amount
...
def validate_concept(concept: str) -> tuple[bool, str | None]:
    ...
    if len(value) > MAX_CONCEPT_LENGTH:
        return False, (
            f"Concepto demasiado largo ({len(value)}/{MAX_CONCEPT_LENGTH} caracteres). Por favor escribe una descripción mas corta:"
        )
