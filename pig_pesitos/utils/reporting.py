...
def build_pdf_report(rows: list[ExpenseRecord], period_label: str, limit_text: str):
    ...
    canvas_doc = canvas.Canvas(pdf_path, pagesize=letter)
    _, height = letter
    margin = 40
    ...
    def draw_page_header():
        nonlocal y_position
        canvas_doc.setFont("Helvetica-Bold", 14)
        canvas_doc.drawString(margin, y_position, f"Reporte detallado ({period_label})")
        y_position -= 25
        canvas_doc.setFont("Helvetica", 11)
        canvas_doc.drawString(margin, y_position, f"Límite mensual: {limit_text}")
        y_position -= 20
